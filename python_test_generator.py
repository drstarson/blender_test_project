...
texture_path = os.path.join(split_path[0],r"assets\textures\table\WoodFine26\WoodFine26\1K") 
...
def create_PBR_mat(object, file_path, texture_name):
    ...
    material.node_tree.links.new(text_diff_node.outputs['Color'],principled_shader.inputs['Base Color'])
    material.node_tree.links.new(text_gloss_node.outputs['Color'],invert_node.inputs['Color'])
    material.node_tree.links.new(invert_node.outputs['Color'],principled_shader.inputs['Roughness'])
    material.node_tree.links.new(text_diff_node.outputs['Color'],principled_shader.inputs['Base Color'])
    material.node_tree.links.new(text_ref_node.outputs['Color'],principled_shader.inputs['Specular'])
    material.node_tree.links.new(text_normal_node.outputs['Color'],normal_map_node.inputs['Color'])
    material.node_tree.links.new(normal_map_node.outputs['Normal'],principled_shader.inputs['Normal'])
    material.node_tree.links.new(principled_shader.outputs['BSDF'],mat_output_node.inputs['Surface'])
    material.node_tree.links.new(tex_coor_node.outputs['Generated'],tex_map_node.inputs['Vector'])
    material.node_tree.links.new(tex_map_node.outputs['Vector'],text_diff_node.inputs['Vector'])
    material.node_tree.links.new(tex_map_node.outputs['Vector'],text_gloss_node.inputs['Vector'])
    material.node_tree.links.new(tex_map_node.outputs['Vector'],text_normal_node.inputs['Vector'])
    material.node_tree.links.new(tex_map_node.outputs['Vector'],text_ref_node.inputs['Vector'])
...
def set_enviroment_tex (file_path,texture_name):
    ...
    world.node_tree.links.new(env_node.outputs['Color'],bg_shader.inputs['Color'])
    world.node_tree.links.new(bg_shader.outputs['Background'],output_node.inputs['Surface'])
